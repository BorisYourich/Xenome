...
def unpaired():
    ...
    prefix_dir = interim_results_dir + "index/"
    xenome_stat_file = sys.argv[5] + "_xenome_stat"
    ...
    try:
        sp = subprocess.Popen(
            ['xenome', 'classify', '-T', xenome_threads, '-P', prefix_dir + sys.argv[3], '--host-name',
             'mouse',
             '--graft-name', 'human', '-i', sys.argv[2]], close_fds=True)
    ...
    while count == 0:
        ...
        try:
            modified_time = datetime.datetime.fromtimestamp(os.path.getmtime("human.fastq"))
        ...
        check_is_size_zero_bytes = os.stat("human.fastq").st_size
    ...
    h1 = subprocess.Popen(['wc', '-l', 'human.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    h2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=h1.stdout, stdout=subprocess.PIPE)
    human = int(h2.communicate()[0]) / 4

    m1 = subprocess.Popen(['wc', '-l', 'mouse.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    m2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=m1.stdout, stdout=subprocess.PIPE)
    mouse = int(m2.communicate()[0]) // 4

    b1 = subprocess.Popen(['wc', '-l', 'both.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    b2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=b1.stdout, stdout=subprocess.PIPE)
    both = int(b2.communicate()[0]) / 4

    n1 = subprocess.Popen(['wc', '-l', 'neither.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    n2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=n1.stdout, stdout=subprocess.PIPE)
    neither = int(n2.communicate()[0]) / 4

    a1 = subprocess.Popen(['wc', '-l', 'ambiguous.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    print("Total = " + str(total) + " Human: " + str(human) + " Mouse: " + str(mouse) + " Both: " + str(
    ...
    try:
        # f = open(interim_results_dir + xenome_stat_file, "a")
...
def paired():
    ...
    try:
        sp = subprocess.Popen(
            ['xenome', 'classify', '-T', xenome_threads, '-P', prefix_dir + sys.argv[3], '--pairs', '--host-name',
             'mouse',
             '--graft-name', 'human', '-i', sys.argv[1], '-i', sys.argv[2]], close_fds=True)
    except Exception as e:
        print('Error executing  xenome classify -> %s' % e)
        sys.exit(1)
    ...
    while count == 0:
        ...
        try:
            modified_time = datetime.datetime.fromtimestamp(os.path.getmtime("human_1.fastq"))
            current_time = datetime.datetime.now()
            back_time = current_time - datetime.timedelta(minutes=5)
            print("[INFO] Current Time: %s Modified Time: %s Back Time: %s" % (current_time, modified_time, back_time))
        except OSError:
        ...
        check_is_size_zero_bytes = os.stat("human_1.fastq").st_size
    ...
    h1 = subprocess.Popen(['wc', '-l', 'human_1.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    h2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=h1.stdout, stdout=subprocess.PIPE)
    human = int(h2.communicate()[0]) / 4

    m1 = subprocess.Popen(['wc', '-l', 'mouse_1.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    m2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=m1.stdout, stdout=subprocess.PIPE)
    mouse = int(m2.communicate()[0]) // 4

    b1 = subprocess.Popen(['wc', '-l', 'both_1.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    b2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=b1.stdout, stdout=subprocess.PIPE)
    both = int(b2.communicate()[0]) / 4

    n1 = subprocess.Popen(['wc', '-l', 'neither_1.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    n2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=n1.stdout, stdout=subprocess.PIPE)
    neither = int(n2.communicate()[0]) / 4

    a1 = subprocess.Popen(['wc', '-l', 'ambiguous_1.fastq'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    a2 = subprocess.Popen(('cut', '-d', ' ', '-f', '1'), stdin=a1.stdout, stdout=subprocess.PIPE)
    ambiguous = int(a2.communicate()[0]) / 4
